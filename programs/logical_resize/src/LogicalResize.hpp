...
{
public:
	LogicalResize() : logical_width_(1), logical_height_(1), physical_width_(1), physical_height_(1) { SetScaleFactor(); }
	LogicalResize(int logical_width, int logical_height)
	...
	void LogicalToPhysical(Rect &inout) const
	{
		Rect input = inout;
		LogicalToPhysical(&inout.x, &inout.y);
		LogicalToPhysical(input.x + input.width + 1, input.y + input.height + 1, inout.width, inout.height);
		inout.width -= inout.x - 1;
		inout.height -= inout.y - 1;
	}
